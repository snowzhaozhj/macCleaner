...
test("R6 空扫描：无孤儿时显示中性空态，不把 fail-closed 结果解释为确定无残留", async ({ page }) => {
  await installTauriMock(page, orphanScanHandlers([]));
  await gotoOrphans(page);

  await expect(page.getByText("未发现孤儿残留。")).toBeVisible();
  await expect(page.getByText(/已卸载应用未留下/)).toHaveCount(0);
...
test("R6 删除失败：命令 reject 后显示本次错误，不残留成功回执", async ({ page }) => {
  const [item] = orphanItems();
  await installTauriMock(page, {
    ...orphanScanHandlers([item]),
    clean_orphans: { error: "删除失败：目标被占用" },
  });
  await gotoOrphans(page);

  await page.getByRole("button", { name: /应用残留 \(Caches\)/ }).click();
  await page.getByRole("checkbox", { name: item.path }).check();
  await page.getByRole("button", { name: /移入废纸篓/ }).click();

  await expect(page.getByRole("alert")).toContainText("清理失败");
  await expect(page.getByRole("alert")).toContainText("目标被占用");
  await expect(page.getByText(/已释放/)).toHaveCount(0);
  await expect(page.getByRole("button", { name: "再次扫描" })).toBeVisible();
});
