...
// skip-fda-guide 计划 U3/R1：Purge 内联跳过块的 FDA 引导按钮（与 Clean 内联块平行的第二份手写副本，
// 可独立漂移，故独立覆盖）——扫描流含 SkippedNoPermission 时 results 相位出现按钮，点击触发 open_fda_settings。
test("权限跳过 FDA 引导：purge 内联跳过区『打开磁盘访问权限设置』触发 open_fda_settings", async ({ page }) => {
  const items = [scanItem(`${FAKE_HOME}/code/web/node_modules`, 500 * MB, { category: "Node.js" })];
  // scanStream 不发跳过事件，手动在 Complete 前插入 SkippedNoPermission（Purge 从流累积 skipped）。
  const events = scanStream(items);
  events.splice(events.length - 1, 0, {
    SkippedNoPermission: { path: `${FAKE_HOME}/Library/Mail` },
  });
  await installTauriMock(page, {
    ...defaultHandlers(),
    scan_purge: { events, result: scanResult(items, [`${FAKE_HOME}/Library/Mail`]) },
  });
  await gotoPurge(page);
  await page.getByRole("button", { name: "开始扫描" }).click();
...
test("AE7 纯 Safe/Moderate 删除：无确认模态，purge 空口令触发，回执+撤销吐司，撤销以 run_id 调 undo", async ({ page }) => {
  const items = devItems();
  const paths = items.map((i) => i.path);
  await installTauriMock(page, {
    ...purgeScanHandlers(items),
    purge: {
      events: cleanStream(paths, 800 * MB),
      result: cleanResponse(paths, 400 * MB, { runId: "purge-run-1" }),
    },
...
test("AE4 Risky 项：默认未预选不入删除批次；显式勾选后须 type-to-confirm，purge 带口令", async ({ page }) => {
  ...
  await installTauriMock(page, {
    ...
    purge: {
      ...
      result: cleanResponse([safe.path, risky.path], 350 * MB),
    },
  });
  await gotoPurge(page);
  await page.getByRole("button", { name: "开始扫描" }).click();
...
test("AE5 隔离（前端面）：clean 与 purge 各自删除各自扫描的项", async ({ page }) => {
  ...
  await installTauriMock(page, {
    ...
    purge: { events: cleanStream(purgePaths, 800 * MB), result: cleanResponse(purgePaths, 400 * MB) },
  });
