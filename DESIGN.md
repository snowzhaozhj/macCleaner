...
**符号轴解耦（best practice）**：严重度形状**独占非三角家族** `● ◆ ✕`（圆=稳 / 菱=留意 / 叉=停），与导航展开符的三角家族 `▶ ▼` 分属两个视觉轴、任何位置都不混淆。一个语义轴占一个视觉家族——曾用的 `▲`（三角）与导航 `▶/▼` 撞家族，已弃用。
...
| `safety.moderate` | `Yellow` `◆` | `oklch(0.78 0.11 90)` `◆` | 形状取非三角，避与导航 `▶▼` 撞家族 |
...
### 6.0 统一行文法（四功能共用的交互词汇）

Clean / Uninstall / Purge / Analyze 的列表行**共用同一套交互符号**，各功能只在自己独有的数据列上有差异（合理缺列，非另一套画法）。列序与含义：

```
<游标高亮> <展开/进入 ▶▼> <复选框 [x]/[ ]> <安全符 ●◆✕> 名称 ······ <大小> <占比条>
```

- **展开/进入**（`▶` 可展开或可进入目录、`▼` 已展开、叶子留空）——导航轴，独占三角家族。
- **复选框**（`[x]/[ ]`，分类头再有半选 `[-]`）——**选择的唯一词汇，四功能一致**；Analyze 亦用它（不再用 `[D]` 后缀），marked 另叠 `state.danger` 色 + 删除线做强化。
- **安全符**（`●◆✕`）——严重度轴，独占非三角家族；只在**分区标题**与**文件项**出现，**分类头不放**（紧邻展开符会撞家族，且与分区标题/名字色/文字标签冗余）。
- **合理缺列**：Analyze 无规则数据 → 无安全符列，改有大小/占比条；Results 家族无占比条。共享列（游标/展开/复选/名称）四功能完全一致。
...
| 列表行（分隔/分类/文件） | `ui/rows.rs` | 见 §6.0；分区标题+文件项带 `●◆✕`、分类头不带；`[x]/[-]/[ ]`复选+`▼/▶`展开 | hover 态、拖选、右键菜单 |
...
| 分析器行（体积条） | `ui/analyzer.rs` | 遵循 §6.0：`▶`进入 + `[x]/[ ]`复选 + 名称 + `█/░`条(`accent.explore`)；marked→`state.danger`+`CROSSED_OUT`，**大文件**(>100MiB **且为文件**)→`state.warning`+名字列 `⚠ ` 前缀，普通文件→`ink`，目录→`accent`(恒定) | 真图形条 |

**符号表（须跨后端保留识别度）**：安全 `●◆✕`（严重度轴，非三角）；展开/进入 `▶ ▼`（导航轴，三角）；复选 `[x] [-] [ ]`；菜单游标 `▶`；分析器体积条 `█ ░`；面包屑分隔 ` / `；spinner `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏`；警告 `⚠`；输入光标 `▏`。
