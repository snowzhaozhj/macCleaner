...
**测量结论（U2，`~/workspace` dry-run，release，暖缓存偏向 old）**：

| | wall | user+sys = CPU-s | avg CPU% |
|---|---|---|---|
| OLD（嵌套池 ~16 线程） | 8.00s | 25.98 | ~325% |
| NEW（U6 串行 dir_size / 4） | 6.3–10.9s（噪声大） | 7.3–9.1 | **~88%** |

- **CPU 工作量降 ~3.2×（−69%）**，avg CPU 从 ~325%（吻合用户观察的 150-200%+）降到 ~88%（亚单核）——达成 R1。
- **墙钟无显著退化**：合成基准 criterion 20 样本对照 −3.5%、p=0.53（不显著），满足 R2；真实树单次跑受系统负载 + 20GB 级大目录支配、噪声过大不可单独判读。

**据数据延后（非本 PR）**：

- **U5（throttle/空转重绘）**：U6 已把总 CPU 降到亚单核，渲染仅占其中一小片，调 throttle 收益不足以justify——延后，除非后续实测重绘仍是热点。
- **U7（减扫描线程数）**：U6 已达标且未退化墙钟；进一步减外层线程只会为无谓的 CPU 需求冒墙钟退化风险——判定不做，保持缺省（外层 4 / 内层串行）。`MC_DIRSIZE_THREADS` 已提供后续调参旋钮。
