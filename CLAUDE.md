...
- **文档目录**：`docs/brainstorms/`（需求）、`docs/ideation/`、`docs/plans/`、`docs/solutions/`（可复用学习，按类别组织并带 `module` / `tags` / `problem_type` frontmatter；实现或调试相关领域前值得搜索）。已解决的问题沉淀到 `docs/solutions/`，领域词汇沉淀到 `CONCEPTS.md`。
