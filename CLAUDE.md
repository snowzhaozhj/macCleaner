...
- **工作流 hook**（`.claude/settings.json`，位于主仓根、非 worktree）：`main` 分支上禁止 Edit/Write；每次编辑 `.rs` 后自动跑 clippy。故不在 `main` 上直接改代码——用分支/worktree。注意：**在 worktree 内该 hook 不生效，需手动 `cargo clippy`**。
