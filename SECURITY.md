...
- **内置规则不引用用户数据路径**：`Documents`、`Desktop`、`Downloads` 等目录不出现在任何内置规则里（契约测试强制，见下）。
- **宽泛通配被刻意窄化**：如 `.gradle` 只精确匹配 `~/.gradle/caches`，绝不整树匹配（否则会误删签名密钥与配置）。

## 隐私与依赖面

- **零遥测**：不采集、不上报任何使用数据，运行时不发起网络请求。
- **内置规则编译进二进制**：`clean_rules.toml` / `purge_rules.toml` 通过 `include_str!` 编译进 `mc`。用户可选地在 `~/.config/mc/rules.toml` 放置**本地叠加规则**（运行时读取、纯本地磁盘，**无任何远程规则拉取**）；其安全模型见下节。
...
内置规则的契约测试是编译期的，管不到用户写在 `~/.config/mc/rules.toml` 里的叠加规则，故加载时施加**运行时护栏**：

- **`preselect` 无条件强制为 `false`**：用户叠加规则永不进入预选集，即 `--yes` 与 TUI 默认勾选**绝不会自动选中它们**——这是防自动删除的主闸。用户可以让规则指向任意路径（含自己的用户数据目录），但删除永远需要在 TUI 里手动勾选并触发，且默认仍是移废纸篓（可恢复）。
- **`DirName` 规则必配非空 `root_markers`**：与内置同标准，防「按目录名整树误爆」。
- **fail-closed**：TOML 解析失败或任一条违反 `DirName` 护栏，则**整个用户文件不加载**（宁可不扩展也不带病加载）。

因此「防自动删除」由 `preselect=false` 强制单点保证，无需再对用户可指向的路径设黑名单。

## 机器可验证的契约

安全边界不靠自觉，由 `crates/core/src/rules.rs` 的单元测试作为**行为契约**在 CI 中持续验证：

- `clean_rules_all_safe`（:271）—— 所有系统缓存规则必须为 Safe。
- `purge_rules_safety_levels`（:283）—— 开发产物按 rubric 分级，可能丢数据/状态的项必须为 Risky。
- `dirname_rules_have_guards`（:433）—— 除 `__pycache__` 外每条按目录名匹配的规则都必须配置项目根守卫。
- `no_rules_reference_user_data_paths`（:501）—— 任何**内置**规则都不得引用 `Documents`/`Desktop`/`Downloads`（契约仅约束内置规则；用户叠加规则的护栏见上节「用户叠加规则的安全模型」）。
