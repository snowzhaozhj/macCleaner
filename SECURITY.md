...
- `clean_rules_all_safe` —— 所有系统缓存规则必须为 Safe。
- `purge_rules_safety_levels` —— 开发产物按 rubric 分级，可能丢数据/状态的项必须为 Risky。
- `dirname_rules_have_guards` —— 除 `__pycache__` 外每条按目录名匹配的规则都必须配置项目根守卫。
- `no_rules_reference_user_data_paths` —— 任何**内置**规则都不得引用 `Documents`/`Desktop`/`Downloads`（契约仅约束内置规则；用户叠加规则的护栏见上节「用户叠加规则的安全模型」）。
